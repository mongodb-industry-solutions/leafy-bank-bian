...
export default function GlMonitor() {
  ...
  const setCol = (key, patch) => setColumns((c) => ({ ...c, [key]: { ...c[key], ...patch } }));
  ...
  const fetchColumn = useCallback(async (key, buildPath, tsField, batchInfo) => {
  ...
  const refreshAll = useCallback(async () => {
    ...
    fetchColumn("tx", () => `transactions${buildParams(p, { limit: 100 })}`, "createdAt", batchInfo);
    fetchColumn("le", () => `ledger-events${buildParams(p, s ? { status: s, limit: 100 } : { limit: 100 })}`, "occurredAt", batchInfo);
    fetchColumn("sl", () => `subledger-entries${buildParams(p, { limit: 100 })}`, "postingDate", batchInfo);
    fetchColumn("jn", () => `journals${buildParams(p, { limit: 100 })}`, "createdAt", batchInfo);
  ...
  const onSelect = (col, idx, item) => {
    setSelected({ col, idx });
    setDetail({ open: true, kind: col, data: item, bg: DETAIL_BG[col], title: DETAIL_TITLE[col](item) });
  };
